#!/usr/bin/env bash
# Build & package Bitbloq Offline release assets (bitbloq-offline 2.0.2)
#
# IMPORTANT (v2.0.2): Bitbloq Offline NO LONGER depends on the external
...
#   ./node_modules/.bin/grunt package-linux        # -> dist/bitbloq_2.0.2_amd64.deb
#                                                 #    dist/BitbloqOffline-2.0.2.AppImage
#   cd dist && zip -r -q bitbloq-offline-linux-2.0.2.zip BitbloqOfflineLinux/ && cd ..
...
#   ./node_modules/.bin/grunt pkg-nsis-win         # -> dist/bitbloq-offline-setup-2.0.2.exe
#   cd dist && 7z a -r bitbloq-offline-windows-2.0.2.zip BitbloqOfflineWin/ && cd ..
...
#   cd dist && zip -r -q bitbloq-offline-mac-2.0.2.zip BitbloqOfflineMac/ && cd ..
#
# ---------------------------------------------------------------------------
# 4. Upload  (once all assets are collected)
# ---------------------------------------------------------------------------
#   gh release create v2.0.2 -t "Bitbloq Offline 2.0.2" -n "..." \
#     dist/bitbloq-offline-linux-2.0.2.zip \
#     dist/bitbloq_2.0.2_amd64.deb \
#     dist/BitbloqOffline-2.0.2.AppImage \
#     dist/bitbloq-offline-windows-2.0.2.zip \
#     dist/bitbloq-offline-setup-2.0.2.exe \
#     dist/bitbloq-offline-mac-2.0.2.zip
#
# NOTE: There is no Web2Board asset to publish. Starting from v2.0.2 Bitbloq
